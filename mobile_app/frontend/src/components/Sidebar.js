...
const Sidebar = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`
        medical-sidebar fixed top-16 left-0 z-50 h-[calc(100vh-4rem)] overflow-y-auto
            ...
            {navigation.map((item) => {
              ...
              return (
                <Link
                  ...
                  onClick={() => setSidebarOpen(false)}
                  className={`
                    flex items-center px-3 py-2 sm:px-3 sm:py-2 rounded-lg text-sm font-medium transition-colors duration-200
                    ${isActive(item.href) 
